...
use multiway_mg::{CycleScreenedMapHierarchy, CycleScreenedMapHierarchyWorkspace, DensePseudoinverse, FactorAggregation, MultiwayError, Preconditioner, SymmetricMapPreconditioner, ThreeWayProblem};
...
fn no_events(stats: Stats) {
    assert_eq!(stats, Stats::default(), "prepared operation touched the allocator");
}
...
fn positive_controls() {
    ...
    assert!(growth.reallocations > 0, "reallocation instrument is inactive");
    println!("positive-controls allocations={} reallocations={}", allocation.allocations, growth.reallocations);
}
...
fn operator_checks() -> Result<()> {
    ...
    let mut out = [0.0;6];
    no_events(measure(|| map.apply_with_workspace(black_box(&rhs),black_box(&mut out),&mut map_workspace))?);
    no_events(measure(|| terminal.solve_into_with_workspace(black_box(&rhs),black_box(&mut out),&mut terminal_workspace))?);
    no_events(measure(|| {
        for _ in 0..64 {
            map.apply_with_workspace(black_box(&rhs),black_box(&mut out),&mut map_workspace)?;
            terminal.solve_into_with_workspace(black_box(&rhs),black_box(&mut out),&mut terminal_workspace)?;
        }
        Ok(())
    })?);
...
fn check_hierarchy(name: &str, hierarchy: &CycleScreenedMapHierarchy, reuse: &mut CycleScreenedMapHierarchyWorkspace) -> Result<()> {
    let rhs: Vec<_> = (0..hierarchy.dimension()).map(|i| ((i as f64 + 0.25)*0.73).sin()).collect();
    let mut reference = vec![0.0;rhs.len()];
    let ordinary = measure(|| hierarchy.apply(black_box(&rhs),black_box(&mut reference)))?;
    assert!(ordinary.allocations > 0, "ordinary allocating control did not allocate");
    ...
    assert_eq!(setup.bytes_allocated,retained,"fresh exclusive payload accounting disagrees with allocator");
    assert_eq!(retained, workspace.traversal_retained_bytes()? + workspace.operator_retained_bytes()?);
    let mut out = vec![f64::NAN;rhs.len()];
    let first = measure(|| hierarchy.apply_with_workspace(black_box(&rhs),black_box(&mut out),&mut workspace))?;
    no_events(first);
    ...
    no_events(measure(|| hierarchy.apply_with_workspace(black_box(&rhs),black_box(&mut out),reuse))?);
    equal_bits(&out,&reference);
    let bytes = reuse.retained_bytes()?;
    let mut bad = vec![23.0;rhs.len()+1];
    let before = GLOBAL.stats();
    assert!(hierarchy.apply_with_workspace(&rhs,&mut bad,reuse).is_err());
    let invalid = GLOBAL.stats()-before;
    no_events(invalid);
    assert!(bad.iter().all(|&x| x == 23.0));
    assert_eq!(reuse.retained_bytes()?,bytes);
    println!("{name}\tdimension={}\tdepth={}\tsetup_allocations={}\tretained_bytes={}\tordinary_allocations={}\tfirst_allocations=0\trepeat64_allocations=0\treallocations=0\tdeallocations=0", hierarchy.dimension(),hierarchy.depth(),setup.allocations,retained,ordinary.allocations);
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    for fixture in fixtures {
        ...
        check_hierarchy(&format!("{}-fresh-weights",fixture.name),&changed,&mut reuse)?;
    }
    let problem = ThreeWayProblem::from_observations([2;3],&[[0,0,0],[1,1,1]],&[1.0,2.0])?;
    let terminal_only = CycleScreenedMapHierarchy::from_maps(problem,Vec::<FactorAggregation>::new(),1.0e-12)?;
    check_hierarchy("disconnected-terminal-only",&terminal_only,&mut reuse)?;
    println!("PASS cases=17 first/repeated/reprepared complete applies and exact fresh exclusive payload");
    Ok(())
}

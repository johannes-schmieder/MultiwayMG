...
fn assert_reference(actual: &PcgTraceResult, expected: &reference::PcgTraceResult) {
    ...
    assert_eq!(actual.preconditioner_applications(), expected.preconditioner_applications());
    assert_eq!(actual.rhs_projection_norm().to_bits(), expected.rhs_projection_norm().to_bits());
    assert_eq!(actual.final_relative_residual().to_bits(), expected.final_relative_residual().to_bits());
...
fn rhs(problem: &ThreeWayProblem, scale: f64) -> Vec<f64> {
    let beta: Vec<_> = (0..problem.dimension()).map(|i| scale * ((i as f64 + 0.5) * 0.71).sin()).collect();
...
fn certify(problem: &ThreeWayProblem, rhs: &[f64], actual: &PcgTraceResult) {
    if actual.converged() {
        ...
        assert!(r <= 1.0e-8 * b.max(1.0), "independent original-operator residual {r} / {b}");
    }
}

#[test]
fn every_public_path_matches_independent_prechange_pcg_on_revealed_fixtures() -> Result<(), fixtures::DynError> {
    ...
    for fixture in fixtures {
        ...
        let hierarchy = CycleScreenedMapHierarchy::from_maps(problem.clone(), fixture.oracle_maps, 1.0e-12)?;
        ...
        for scale in [1.0, -2.0, 0.0, 0.5] {
            ...
            let expected = reference::solve_projected_pcg_traced(&problem, &rhs, &hierarchy, old_options(options))?;
            let owned = solve_projected_pcg_traced(&problem, &rhs, &hierarchy, options)?;
            assert_reference(&owned, &expected);
            let owned_hierarchy = solve_projected_pcg_traced_with_hierarchy_workspace(&problem, &rhs, &hierarchy, options, &mut hierarchy_storage)?;
            assert_reference(&owned_hierarchy, &expected);
            let generic = solve_projected_pcg_traced_with_workspace(&problem, &rhs, &hierarchy, options, &mut storage)?.to_owned();
            assert_reference(&generic, &expected);
            let prepared = solve_projected_pcg_traced_with_workspaces(&problem, &rhs, &hierarchy, options, &mut storage, &mut hierarchy_storage)?.to_owned();
...
fn zero_limit_and_explicit_owned_copy_preserve_result_lifetimes() {
    ...
    let first = solve_projected_pcg_traced_with_workspaces(&problem, &zero, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap().to_owned();
    ...
    let expected = reference::solve_projected_pcg_traced(&problem, &input, &hierarchy, old_options(limit)).unwrap();
    let actual = solve_projected_pcg_traced_with_workspaces(&problem, &input, &hierarchy, limit, &mut storage, &mut hierarchy_storage).unwrap().to_owned();
    ...
    assert_eq!(first.samples().len(), 1);
    let converged = solve_projected_pcg_traced_with_workspaces(&problem, &input, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap().to_owned();
...
fn validation_is_transactional_and_preparation_is_explicit() {
    ...
    let options = PcgTraceOptions { max_iterations: 1, ..PcgTraceOptions::default() };
    let mut storage = PcgTraceWorkspace::try_new(&owner, options).unwrap();
    let input = rhs(&owner, 1.0);
    let before = format!("{storage:?}");
    assert!(solve_projected_pcg_traced_with_workspace(&other, &input, &hierarchy, options, &mut storage).is_err());
    assert_eq!(format!("{storage:?}"), before);
    let larger_budget = PcgTraceOptions { max_iterations: storage.trace_capacity(), ..options };
    assert!(solve_projected_pcg_traced_with_workspace(&owner, &input, &hierarchy, larger_budget, &mut storage).is_err());
    assert_eq!(format!("{storage:?}"), before);
    let impossible = PcgTraceOptions { max_iterations: usize::MAX, ..options };
    assert!(storage.try_prepare_for(&owner, impossible).is_err());
    assert_eq!(format!("{storage:?}"), before);
    for options in [
        PcgTraceOptions { relative_tolerance: f64::NAN, ..options },
        PcgTraceOptions { absolute_tolerance: -1.0, ..options },
        PcgTraceOptions { relative_tolerance: 0.0, absolute_tolerance: 0.0, ..options },
        PcgTraceOptions { max_iterations: 0, ..options },
    ] {
        let expected = reference::solve_projected_pcg_traced(&owner, &input, &hierarchy, old_options(options)).unwrap_err();
        let actual = solve_projected_pcg_traced_with_workspace(&owner, &input, &hierarchy, options, &mut storage).unwrap_err();
        assert_eq!(actual.to_string(), expected.to_string());
        assert_eq!(format!("{storage:?}"), before);
    }
    for length in [input.len() - 1, input.len() + 1] {
        let wrong = vec![1.0; length];
        let expected = reference::solve_projected_pcg_traced(&owner, &wrong, &hierarchy, old_options(options)).unwrap_err();
        let actual = solve_projected_pcg_traced_with_workspace(&owner, &wrong, &hierarchy, options, &mut storage).unwrap_err();
        assert_eq!(actual.to_string(), expected.to_string());
        assert_eq!(format!("{storage:?}"), before);
    }
    let wrong_hierarchy = self::hierarchy(&problem(2, 1.0));
    let expected = reference::solve_projected_pcg_traced(&owner, &input, &wrong_hierarchy, old_options(options)).unwrap_err();
    let actual = solve_projected_pcg_traced_with_workspace(&owner, &input, &wrong_hierarchy, options, &mut storage).unwrap_err();
    assert_eq!(actual.to_string(), expected.to_string());
    assert_eq!(format!("{storage:?}"), before);
    let mut hierarchy_storage = CycleScreenedMapHierarchyWorkspace::new();
    for levels in [5, 1, 4, 2, 5] {
        ...
        let expected = reference::solve_projected_pcg_traced(&next, &input, &next_hierarchy, old_options(options)).unwrap();
        let actual = solve_projected_pcg_traced_with_workspaces(&next.clone(), &input, &next_hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap().to_owned();
        assert_reference(&actual, &expected);
        assert!(storage.retained_bytes().unwrap() >= PcgTraceWorkspace::required_bytes(&next, options).unwrap());
    }
}
...
impl Preconditioner for Interrupt {
    ...
    fn apply(&self, rhs: &[f64], out: &mut [f64]) -> Result<(), MultiwayError> {
        ...
        if calls == self.at {
            ...
            return Err(MultiwayError::PcgBreakdown { iteration: calls, message: "injected preconditioner error".to_owned() });
        }
...
fn numerical_errors_and_preconditioner_unwind_leave_reusable_scratch() {
    ...
    for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        for all in [false, true] {
            ...
            if all { wrong.fill(bad); } else { wrong[0] = bad; }
            let expected = reference::solve_projected_pcg_traced(&problem, &wrong, &hierarchy, old_options(options)).unwrap_err();
            let actual = solve_projected_pcg_traced_with_workspaces(&problem, &wrong, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap_err();
            assert_eq!(actual.to_string(), expected.to_string());
            assert_eq!(hierarchy_storage.retained_bytes().unwrap(), 0);
    ...
    let expected = reference::solve_projected_pcg_traced(&problem, &huge, &hierarchy, old_options(options)).unwrap_err();
    let actual = solve_projected_pcg_traced_with_workspaces(&problem, &huge, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap_err();
    assert_eq!(actual.to_string(), expected.to_string());
    for panic in [false, true] {
        for at in [1, 2] {
            let mock = Interrupt { dimension: problem.dimension(), calls: std::cell::Cell::new(0), at, panic };
            let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                assert!(solve_projected_pcg_traced_with_workspace(&problem, &input, &mock, options, &mut storage).is_err());
            }));
            assert_eq!(outcome.is_err(), panic);
            assert_eq!(mock.calls.get(), at);
            let expected = reference::solve_projected_pcg_traced(&problem, &input, &hierarchy, old_options(options)).unwrap();
            let actual = solve_projected_pcg_traced_with_workspaces(&problem, &input, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap().to_owned();
            assert_reference(&actual, &expected);
            assert_eq!(storage.retained_bytes().unwrap(), bytes);
        }
    }
    ...
    std::thread::scope(|scope| {
        ...
        for _ in 0..4 {
            threads.push(scope.spawn(|| {
                ...
                for _ in 0..4 {
                    let result = solve_projected_pcg_traced_with_workspaces(&problem, &input, &hierarchy, options, &mut storage, &mut hierarchy_storage).unwrap();

...
fn check(topology: &PreparedThreeWayTopology, input: WeightFrameInput<'_>) -> Result<()> {
    ...
    let error = ThreeWayWeightFrame::try_new_with_budget(
        topology,
        input,
    ...
    assert!(
        matches!(error, IncidenceError::WeightFrameBudgetExceeded { required, .. } if required == report.total_payload_bytes)
    );
    let before = GLOBAL.stats();
    let frame = black_box(ThreeWayWeightFrame::try_new_with_budget(
        topology,
        input,
    ...
    assert_eq!(
        ...
        topology.topology().total_levels() * 8
    );
    ...
    for _ in 0..64 {
        ...
        assert_eq!(
            ThreeWayWeightFrame::setup_payload_report(topology, input, old_bytes)?,
            report
        );
    ...
    let before = GLOBAL.stats();
    let wrong_frame = frame
        .copy_weights_into(topology, old.binding(), &mut output)
        .unwrap_err();
    let wrong_topology = frame
        .copy_weights_into(&foreign, binding, &mut output)
        .unwrap_err();
    let wrong_size = frame
        .copy_weights_into(topology, binding, &mut output[..0])
        .unwrap_err();
    no_events(GLOBAL.stats() - before);
    assert!(matches!(
        wrong_frame,
        IncidenceError::WeightFrameBindingMismatch
    ));
    assert!(matches!(
        wrong_topology,
    ...
    println!(
        "weight-frame input={:?} tuples={} new_setup={} retained={retained} old_live={old_bytes} live_budget={} first/repeat/read/copy/reject_allocations=0 release=exact rebuild16=balanced",
        input.kind(),
...
fn failed_construction_releases_arrays() -> Result<()> {
    ...
    for (topology, input, expected_allocations) in [
        (
            &observed,
            WeightFrameInput::Observations(&[1.0, f64::NAN]),
            0,
        ),
    ...
    println!(
        "weight-frame failed input/duplicate/degree construction allocations=0/1/4 all releases balanced"
    );
    Ok(())
}
...
pub(super) fn run() -> Result<()> {
    ...
    for (counts, rows) in cases {
        ...
        let tuples: Vec<_> = (0..observed.topology().tuple_count())
            .map(|i| 0.5 + (i % 7) as f64)
            .collect();
        for input in [
            WeightFrameInput::Observations(&observations),
        ...
        for topology in [&observed, &collapsed] {
            for input in [
                WeightFrameInput::Tuples(&tuples),
    ...
    println!(
        "PASS weight-frames cases=24 exact setup/retained/destruction/overlap; allocation-free reads and static rejection"
    );
    Ok(())
}

...
fn coarse(map: &PreparedCoarseTupleMap<'_>, parent: &ThreeWayWeightFrame<'_>) -> Result<()> {
    ...
    let budget = |maximum_payload_bytes| WeightReplayPayloadBudget {
        maximum_payload_bytes,
        additional_live_payload_bytes: old_bytes,
    };
    let before = GLOBAL.stats();
    let _error = CoarseWeightReplay::try_new_with_budget(
        map,
        parent,
        budget(report.total_payload_bytes - 1),
    )
    .unwrap_err();
    no_events(GLOBAL.stats() - before);
    let before = GLOBAL.stats();
    let replay = black_box(CoarseWeightReplay::try_new_with_budget(
        map,
        parent,
        budget(report.total_payload_bytes),
    )?);
    ...
    assert_eq!(
        ...
        map.coarse().topology().total_levels() * 8
    );
    ...
    let equal =
        ThreeWayWeightFrame::try_new(map.source(), WeightFrameInput::Tuples(parent.weights()))?;
    ...
    for _ in 0..64 {
        ...
        assert_eq!(
            CoarseWeightReplay::setup_payload_report(map, parent, old_bytes)?,
            report
        );
    ...
    assert!(
        replay
            .copy_weights_into(&alternative, parent, &mut out)
            .is_err()
    );
    assert!(replay.copy_weights_into(map, &equal, &mut out).is_err());
    assert!(
        replay
            .copy_weights_into(map, parent, &mut out[..0])
            .is_err()
    );
    no_events(GLOBAL.stats() - before);
    ...
    for step in 0..16 {
        for (i, weight) in weights.iter_mut().enumerate() {
            *weight = parent.weights()[i]
                * (1.0 + step as f64 / 16.0)
                * if (i + step) % 5 == 0 { 1024.0 } else { 1.0 };
        }
        let before = GLOBAL.stats();
    ...
    equal_bits(&out, &saved);
    println!(
        "coarse-replay fine_tuples={} coarse_tuples={} new_setup={} retained={retained} live_budget={} read/copy/reject_allocations=0 release=exact changed16=balanced",
        parent.weights().len(),
        out.len(),
        report.new_arrays_payload_bytes,
...
fn pair(map: &PreparedPairEdgeMap<'_>, parent: &ThreeWayWeightFrame<'_>) -> Result<()> {
    ...
    let before = GLOBAL.stats();
    let _error = PairConductanceReplay::try_new_with_budget(
        map,
        parent,
        budget(report.total_payload_bytes - 1),
    )
    ...
    no_events(GLOBAL.stats() - before);
    let before = GLOBAL.stats();
    let replay = black_box(PairConductanceReplay::try_new_with_budget(
        map,
        parent,
        budget(report.total_payload_bytes),
    )?);
    ...
    for _ in 0..64 {
        ...
        assert_eq!(
            PairConductanceReplay::setup_payload_report(map, parent, old_bytes)?,
            report
        );
        assert_eq!(replay.retained_payload_bytes()?, retained);
    ...
    assert!(
        replay
            .copy_conductances_into(&alternative, parent, &mut out)
            .is_err()
    );
    assert!(
        replay
            .copy_conductances_into(map, &equal, &mut out)
            .is_err()
    );
    assert!(
        replay
            .copy_conductances_into(map, parent, &mut out[..0])
            .is_err()
    );
    assert!(
        replay
            .write_edges_into(&alternative, parent, &mut edges)
            .is_err()
    );
    assert!(replay.write_edges_into(map, &equal, &mut edges).is_err());
    assert!(
        replay
            .write_edges_into(map, parent, &mut edges[..0])
            .is_err()
    );
    no_events(GLOBAL.stats() - before);
    equal_bits(&out, &saved);
    ...
    equal_bits(&out, &saved);
    println!(
        "pair-replay pair={:?} fine_tuples={} edges={} new_setup={} retained={retained} live_budget={} read/copy/write/reject_allocations=0 release=exact changed16=balanced",
        map.pair(),
        parent.weights().len(),
        out.len(),
        report.new_arrays_payload_bytes,
...
pub(super) fn run() -> Result<()> {
    let cases = [
        ...
        (
            [4; 3],
            (0..128)
                .map(|i| [(i % 4) as u32, ((i / 4) % 4) as u32, ((i / 16) % 4) as u32])
                .collect(),
        ),
    ];
    let mut checked = 0;
    for (counts, rows) in cases {
        ...
        for (t, input) in [
            (&observed, WeightFrameInput::UnitObservations),
            (&collapsed, WeightFrameInput::UnitTuples),
        ] {
            let frame = ThreeWayWeightFrame::try_new(t, input)?;
            let a = if t.component_factor_sizes().len() > 1 {
            ...
            for p in FactorPair::ALL {
                pair(&PreparedPairEdgeMap::try_new(t, p)?, &frame)?;
            }
            checked += 1;
        }
    }
    ...
    println!(
        "PASS weight-replay coarse=8 pair=24 exact lifetimes; changed-frame cycles balanced; reads/static rejection allocate nothing"
    );
    Ok(())
}

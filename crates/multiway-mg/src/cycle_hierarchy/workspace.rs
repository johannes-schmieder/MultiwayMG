...
impl CycleScreenedMapHierarchyWorkspace {
    ...
    pub fn try_prepare_for(&mut self, hierarchy: &CycleScreenedMapHierarchy) -> Result<(), MultiwayError> {
    ...
    pub fn retained_bytes(&self) -> Result<usize, MultiwayError> {
        self.traversal_retained_bytes()?.checked_add(self.operator_retained_bytes()?).ok_or_else(size_overflow)
    }
    ...
    pub fn traversal_retained_bytes(&self) -> Result<usize, MultiwayError> {
        let descriptors = self.buffers.capacity().checked_mul(core::mem::size_of::<Vec<f64>>()).ok_or_else(size_overflow)?;
        self.buffers.iter().try_fold(descriptors, |total, buffer| {
            let bytes = buffer.capacity().checked_mul(core::mem::size_of::<f64>()).ok_or_else(size_overflow)?;
    ...
    fn prepare_vectors(&mut self, problems: &[ThreeWayProblem]) -> Result<(), MultiwayError> {
        ...
        if count > self.buffers.len() {
            self.buffers.try_reserve_exact(count - self.buffers.len()).map_err(allocation_error)?;
        ...
        for (level, pair) in problems.windows(2).enumerate() {
            ...
            for (buffer, length) in self.buffers[start..start + FRAME_BUFFERS].iter_mut().zip([fine, fine, coarse, coarse, fine, fine, fine]) {
...
impl CycleScreenedMapHierarchy {
    ...
    pub fn workspace_required_bytes(&self) -> Result<usize, MultiwayError> {
        ...
        let descriptors = count.checked_mul(core::mem::size_of::<Vec<f64>>()).ok_or_else(size_overflow)?;
        let values = self.problems.windows(2).try_fold(self.dimension(), |total, pair| {
            let fine = pair[0].dimension().checked_mul(5).ok_or_else(size_overflow)?;
            let coarse = pair[1].dimension().checked_mul(2).ok_or_else(size_overflow)?;
            total.checked_add(fine).and_then(|n| n.checked_add(coarse)).ok_or_else(size_overflow)
        })?;
        let vectors = values.checked_mul(core::mem::size_of::<f64>()).ok_or_else(size_overflow)?;
        descriptors.checked_add(vectors).and_then(|n| n.checked_add(operators::required_bytes(self).ok()?)).ok_or_else(size_overflow)
    }
    ...
    pub fn application_workspace(&self) -> Result<CycleScreenedMapHierarchyWorkspace, MultiwayError> {
    ...
    pub fn apply_with_workspace(&self, rhs: &[f64], out: &mut [f64], workspace: &mut CycleScreenedMapHierarchyWorkspace) -> Result<(), MultiwayError> {
        if rhs.len() != self.dimension() {
            return Err(crate::error::dimension("CycleScreenedMapHierarchy::apply rhs", self.dimension(), rhs.len()));
        }
        if out.len() != self.dimension() {
            return Err(crate::error::dimension("CycleScreenedMapHierarchy::apply output", self.dimension(), out.len()));
        }
        workspace.try_prepare_for(self)?;
        let (solution, scratch) = workspace.buffers.split_first_mut().expect("prepared result buffer");
        self.apply_level_into(0, rhs, solution, scratch, &mut workspace.operators.levels[..self.problems.len()], &mut workspace.operators.terminal)?;
    ...
    fn apply_level_into(&self, level: usize, rhs: &[f64], solution: &mut [f64], scratch: &mut [Vec<f64>], operator_levels: &mut [LevelWorkspace], terminal: &mut DensePseudoinverseWorkspace) -> Result<(), MultiwayError> {
        let problem = &self.problems[level];
        if rhs.len() != problem.dimension() {
            return Err(crate::error::dimension("CycleScreenedMapHierarchy::apply_level", problem.dimension(), rhs.len()));
        }
        let (state, children) = operator_levels.split_first_mut().expect("prepared operator level");
        solution.fill(0.0);
        if level == self.aggregations.len() {
            self.terminal.solve_into_with_workspace(rhs, solution, terminal)?;
            problem.components().project_structural_range_with_workspace(solution, &mut state.projection)?;
            return Ok(());
        }
        let (frame, child_scratch) = scratch.split_at_mut(FRAME_BUFFERS);
        let [compatible_rhs, residual, coarse_rhs, coarse_solution, prolonged, post_residual, post] = frame else {
        ...
        problem.components().project_structural_range_with_workspace(compatible_rhs, &mut state.projection)?;
        ...
        coarse_problem.components().project_structural_range_with_workspace(coarse_rhs, &mut children[0].projection)?;
        self.apply_level_into(level + 1, coarse_rhs, coarse_solution, child_scratch, children, terminal)?;
        prolonged.fill(0.0);
...
impl Preconditioner for CycleScreenedMapHierarchy {
    fn dimension(&self) -> usize { self.finest_problem().dimension() }
...
fn required_buffer_count(depth: usize) -> Result<usize, MultiwayError> {
    depth.checked_mul(FRAME_BUFFERS).and_then(|n| n.checked_add(1)).ok_or_else(size_overflow)
}

fn resize_buffer(buffer: &mut Vec<f64>, length: usize) -> Result<(), MultiwayError> {
    if length > buffer.len() { buffer.try_reserve_exact(length - buffer.len()).map_err(allocation_error)?; }
    buffer.resize(length, 0.0);
    Ok(())
}

fn allocation_error(source: std::collections::TryReserveError) -> MultiwayError {
    MultiwayError::WorkspaceAllocation { context: CONTEXT, source }
}
fn size_overflow() -> MultiwayError { MultiwayError::WorkspaceSizeOverflow { context: CONTEXT } }
...
mod tests {
    ...
    fn hierarchy() -> CycleScreenedMapHierarchy {
        let tuples: Vec<_> = (0..4).flat_map(|i| (0..4).flat_map(move |j| (0..4).map(move |k| [i,j,k]))).collect();
        let problem = ThreeWayProblem::from_observations([4;3], &tuples, &vec![1.0;tuples.len()]).unwrap();
        let maps = vec![FactorAggregation::consecutive_halving([4;3]).unwrap(), FactorAggregation::consecutive_halving([2;3]).unwrap()];
    ...
    fn accounting_includes_descriptors_and_growth_is_checked() {
        ...
        let traversal = workspace.buffers.capacity() * core::mem::size_of::<Vec<f64>>() + workspace.buffers.iter().map(|v| v.capacity()*8).sum::<usize>();
        assert_eq!(workspace.traversal_retained_bytes().unwrap(), traversal);
        assert_eq!(workspace.retained_bytes().unwrap(), traversal + workspace.operators.retained_bytes().unwrap());
        assert!(workspace.retained_bytes().unwrap() >= hierarchy.workspace_required_bytes().unwrap());
    ...
    fn poisoned_scratch_and_nested_unwind_do_not_lose_buffers() {
        ...
        for buffer in &mut workspace.buffers { buffer.fill(f64::NAN); }
        let caught = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            hierarchy.apply_level_into(0,&rhs,solution,&mut scratch[..FRAME_BUFFERS],&mut workspace.operators.levels,&mut workspace.operators.terminal).unwrap();
        }));

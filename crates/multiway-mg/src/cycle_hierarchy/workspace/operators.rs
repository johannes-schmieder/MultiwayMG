...
impl OperatorWorkspaces {
    pub(super) fn prepare(
        ...
        if count > self.levels.len() {
            self.levels
                .try_reserve_exact(count - self.levels.len())
    ...
    pub(super) fn retained_bytes(&self) -> Result<usize, MultiwayError> {
        let descriptors = self
            .levels
            .capacity()
            .checked_mul(core::mem::size_of::<LevelWorkspace>())
            .ok_or_else(size_overflow)?;
        let initial = descriptors
            .checked_add(self.terminal.retained_bytes()?)
            .ok_or_else(size_overflow)?;
        self.levels.iter().try_fold(initial, |total, level| {
            let map = level
                .map
                .as_ref()
                .map_or(Ok(0), SymmetricMapWorkspace::retained_bytes)?;
            total
                .checked_add(level.projection.retained_bytes())
                .and_then(|n| n.checked_add(map))
                .ok_or_else(size_overflow)
        })
    }
}
...
pub(super) fn required_bytes(
    ...
    let descriptors = hierarchy
        ...
        .ok_or_else(size_overflow)?;
    let initial = descriptors
        .checked_add(hierarchy.terminal.workspace_required_bytes()?)
        .ok_or_else(size_overflow)?;
    hierarchy
        .problems
        .iter()
        .enumerate()
        .try_fold(initial, |total, (level, problem)| {
            let projection = problem.components().projection_workspace_required_bytes()?;
            let map = hierarchy.smoothers.get(level).map_or(
                Ok(0),
                crate::SymmetricMapPreconditioner::workspace_required_bytes,
            )?;
            total

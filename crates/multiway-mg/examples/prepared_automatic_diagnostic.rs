//! Separate explicit-layout diagnostic probe; the frozen scalar probe is unchanged.
...
const LAYOUTS: [(&str, PreparedAutomaticLayout); 5] = [
    ("scalar", PreparedAutomaticLayout::Scalar),
    ("fine-row", PreparedAutomaticLayout::FineRow),
    ("all-row", PreparedAutomaticLayout::AllRow),
    ("fine-image", PreparedAutomaticLayout::FineImage),
    ("all-image", PreparedAutomaticLayout::AllImage),
];
...
// This record has the same layout in instrumented and uninstrumented builds.
struct Diagnostic {
    valid: bool,
    elapsed: u128,
    unattributed: u128,
...
fn tls_bytes() -> usize {
    ...
    {
        automatic_profiling::thread_local_payload_bytes()
    }
...
struct Record {
    layout: PreparedAutomaticLayoutProgress,
...
impl Record {
    fn new() -> Self {
        Self {
            layout: PreparedAutomaticLayoutProgress::default(),
            diagnostic: None,
            stage: "cli",
            phases: [0; 6],
            total: 0,
...
fn baseline(
    ...
    let groups = if r.layout.requested_layout == PreparedAutomaticLayout::Scalar {
        None
    } else {
        #[cfg(feature = "profiling")]
        let _span = automatic_profiling::span(automatic_profiling::Phase::GlobalGrouping);
        r.layout.grouping_attempts += 1;
        let result = (|| -> Result<_> {
            let other = r.caller + frame.retained_payload_bytes()?;
            r.admit(
                multiway_incidence::PreparedTupleGrouping::setup_payload_bound(
                    frame.topology(),
                    other,
                )?,
            )?;
            let groups = multiway_incidence::PreparedTupleGrouping::try_new_with_budget(
                frame.topology(),
                MAX_PAYLOAD,
                other,
            )?;
            r.admit(r.fine + r.caller + groups.retained_payload_bytes()?)?;
            Ok(groups)
        })();
        match result {
            Ok(groups) => {
                r.layout.completed_groupings = 1;
                r.layout.grouped_levels_built = 1;
                r.layout.maximum_grouping_payload_bytes = groups.retained_payload_bytes()?;
                Some(groups)
            }
            Err(error) => {
                r.layout.rejected_groupings = 1;
                r.layout.last_rejected_grouping = Some(PreparedAutomaticGroupingLocation {
                    component: None,
                    scope: PreparedAutomaticGroupingScope::GlobalBaseline,
                });
                return Err(error);
            }
        }
    ...
    let owner = match &groups {
        Some(g) => owner.with_grouping(g, GroupedGramianMode::RowGather)?,
        None => owner,
    };
    let mut ws = {
        #[cfg(feature = "profiling")]
        let _span = automatic_profiling::span(automatic_profiling::Phase::GlobalWorkspace);
        let opts = options().lsmr;
        r.admit(PreparedLsmrWorkspace::setup_payload_bound(
            &owner, opts, r.caller,
        )?)?;
        let ws = PreparedLsmrWorkspace::try_new_with_payload_budget(
            &owner,
            opts,
            MAX_PAYLOAD,
            r.caller,
    ...
    {
        #[cfg(feature = "profiling")]
        let _span = automatic_profiling::span(automatic_profiling::Phase::GlobalSolve);
...
fn driver(
    ...
    if route == "automatic" || route == "components-map" {
        let mut o = options();
        ...
        let additional = r.caller
            ...
            - size_of_val(x)
            - size_of_val(reports);
        let outcome = solve_prepared_automatic_batch_into_with_layout(
            frame,
        ...
        baseline(frame, input, x, reports, kind, r)
    }
}
fn run(route: &str, reader: &mut impl Read, r: &mut Record) -> Result {
    ...
    #[cfg(feature = "profiling")]
    let result = {
        let (result, profile) = automatic_profiling::collect(|| {
            driver(route, &frame, &input, &mut x, &mut reports, r)
        })?;
        assert_eq!(REGIONS, automatic_profiling::PHASES.map(|p| p.name()));
        r.diagnostic = Some(Diagnostic {
            valid: profile.valid,
            elapsed: profile.elapsed_ns,
            unattributed: profile.unattributed_ns,
            regions: profile.regions.map(|r| (r.calls, r.elapsed_ns)),
        });
        result
    };
    #[cfg(not(feature = "profiling"))]
    let result = driver(route, &frame, &input, &mut x, &mut reports, r);
...
fn emit(route: &str, r: &Record, outcome: &Result) {
    ...
    let g = r.layout;
    println!("layout\t{:?}", g.requested_layout);
    println!(
        "grouping\t{}\t{}\t{}\t{}\t{}\t{}",
        g.grouping_attempts,
        g.completed_groupings,
        g.rejected_groupings,
        g.grouped_levels_built,
        g.maximum_grouping_payload_bytes,
        g.maximum_tuple_image_len
    );
    if let Some(location) = g.last_rejected_grouping {
        println!(
            "grouping_rejection\t{}\t{:?}",
            location
                .component
                .map_or("NA".to_owned(), |i| i.to_string()),
            location.scope
        );
    }
    if let Some(d) = &r.diagnostic {
        ...
        for (name, (calls, elapsed)) in REGIONS.iter().zip(d.regions) {
            println!("region\t{name}\t{calls}\t{elapsed}");
        }
    } else {
        println!(
            "diagnostic\tNA\tNA\tNA\t{}\t{}",
...
fn main() {
    ...
    let result = if args.next().is_some() {
        Err("exactly one route and layout required".into())
    } else if let Some((_, layout)) = LAYOUTS.iter().find(|(name, _)| *name == selected) {
        r.layout.requested_layout = *layout;
        run(&route, &mut io::stdin().lock(), &mut r)
    } else {
        Err("unknown or missing explicit layout".into())
    };
...
mod tests {
    ...
    fn every_route_certifies_mixed_columns_and_preserves_input_boundary() {
        for route in ROUTES {
            let mut reference = Record::new();
            run(route, &mut &input(32)[..], &mut reference).unwrap();
            for (_, layout) in LAYOUTS {
                let mut r = Record::new();
                r.layout.requested_layout = layout;
                run(route, &mut &input(32)[..], &mut r).unwrap();
                for (actual, expected) in r.columns.iter().zip(reference.columns) {
                    let actual = actual.unwrap();
                    let expected = expected.unwrap();
                    assert!(actual.report.accepted);
                ...
                assert_eq!(r.layout.requested_layout, layout);
                assert_eq!(
                    r.layout.grouping_attempts,
                    usize::from(
                        route.starts_with("global-") && layout != PreparedAutomaticLayout::Scalar
                    )
                );
                assert_eq!(r.layout.maximum_tuple_image_len, 0);
                #[cfg(feature = "profiling")]
                {
                    ...
                    assert_eq!(
                        d.elapsed,
                        d.unattributed + d.regions.iter().map(|r| r.1).sum::<u128>()
                    );
                }
                #[cfg(not(feature = "profiling"))]

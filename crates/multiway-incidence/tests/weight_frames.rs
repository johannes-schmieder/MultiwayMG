...
fn check_observations(counts: [usize; 3], rows: &[[u32; 3]], weights: &[f64]) {
    ...
    let frame =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Observations(weights)).unwrap();
    let fresh = ThreeWayProblem::from_observations(counts, rows, weights).unwrap();
    let mut grouped = BTreeMap::<[u32; 3], (f64, f64)>::new();
    for (&row, &value) in rows.iter().zip(weights) {
        add(grouped.entry(row).or_default(), value);
    }
    let expected: Vec<_> = grouped
        .values()
    ...
    for (&tuple, &value) in topology.topology().tuples().iter().zip(&expected) {
        for factor in 0..3 {
            add(
                &mut degree[topology.topology().global_index(factor, tuple[factor])],
                value,
            );
        }
    }
    let degree: Vec<_> = degree
        .into_iter()
        .map(|(sum, correction)| sum + correction)
        .collect();
    ...
    let from_tuples =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&expected)).unwrap();
    ...
    assert_eq!(
        frame.validation_report().input_kind,
        WeightFrameInputKind::Observations
    );
    ...
    assert_eq!(
        frame.validation_report().component_count,
        topology.component_factor_sizes().len()
    );
    for (component, range) in frame.component_ranges().iter().enumerate() {
        let component_weights: Vec<_> = topology
            .topology()
            .tuples()
            .iter()
            .zip(&expected)
            .filter_map(|(tuple, &weight)| {
                (topology.component_labels()[tuple[0] as usize] == component).then_some(weight)
            })
            .collect();
        let component_degrees: Vec<_> = topology
            .component_labels()
            .iter()
            .zip(&degree)
            .filter_map(|(&label, &value)| (label == component).then_some(value))
            .collect();
        assert_eq!(
        ...
        assert_eq!(
            ...
            component_weights.iter().copied().fold(0.0, f64::max)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            component_degrees.iter().copied().fold(0.0, f64::max)
        );
    }
}
...
fn duplicate_order_and_all_small_valid_supports_match_independent_reference() {
    ...
    for mask in 1..256 {
        let rows: Vec<_> = universe
            ...
            .collect();
        if (0..3).any(|f| (0..2).any(|l| !rows.iter().any(|t| t[f] == l))) {
            continue;
        }
        ...
        let weights: Vec<_> = (0..duplicated.len())
            .map(|i| [1.0e16, 1.0, 1.0, 0.25, 1.0e-200][i % 5])
            .collect();
...
fn unit_observations_and_unit_tuples_are_explicitly_different() {
    ...
    let unit_obs =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::UnitObservations).unwrap();
    let ones_obs =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Observations(&[1.0; 3])).unwrap();
    let unit_tuples =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::UnitTuples).unwrap();
    let ones_tuples =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&[1.0; 2])).unwrap();
    bits(unit_obs.weights(), &[1.0, 2.0]);
    bits(unit_obs.diagonal(), &[1.0, 2.0, 1.0, 2.0, 1.0, 2.0]);
    bits(unit_obs.weights(), ones_obs.weights());
    bits(
        unit_obs.square_root_weights(),
        ones_obs.square_root_weights(),
    );
    bits(unit_obs.diagonal(), ones_obs.diagonal());
    bits(unit_tuples.weights(), &[1.0, 1.0]);
    bits(unit_tuples.diagonal(), ones_tuples.diagonal());
    assert_eq!(
        unit_obs.validation_report().input_kind,
        WeightFrameInputKind::UnitObservations
    );
    assert_eq!(
        unit_tuples.validation_report().input_kind,
        WeightFrameInputKind::UnitTuples
    );
    let collapsed =
        PreparedThreeWayTopology::try_from_collapsed([2; 3], &[[0; 3], [1; 3]]).unwrap();
    for input in [
        WeightFrameInput::UnitObservations,
        WeightFrameInput::Observations(&[1.0; 2]),
    ] {
        assert!(matches!(
            ThreeWayWeightFrame::try_new(&collapsed, input),
            Err(IncidenceError::WeightFrameObservationLayoutRequired)
        ));
    }
...
fn generation_checks_distinguish_equal_and_changed_frames_before_output_mutation() {
    ...
    let changed =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&[2.0])).unwrap();
    ...
    for target in [&equal, &changed] {
        assert!(matches!(
            target.copy_weights_into(&topology, token, &mut output),
            Err(IncidenceError::WeightFrameBindingMismatch)
        ));
        assert_eq!(output, [23.0]);
    }
    assert!(matches!(
        first.copy_weights_into(&foreign, token, &mut output),
        Err(IncidenceError::TopologyBindingMismatch)
    ));
    assert!(
        first
            .copy_weights_into(&topology, token, &mut output[..0])
            .is_err()
    );
    assert_eq!(output, [23.0]);
    first
        .copy_weights_into(&topology, token, &mut output)
        .unwrap();
...
fn finite_positive_extremes_are_preserved_but_duplicate_and_degree_overflow_reject() {
    ...
    for invalid in [0.0, -0.0, -1.0, f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        assert!(matches!(
            ThreeWayWeightFrame::try_new(
                &topology,
                WeightFrameInput::Observations(&[1.0, invalid])
            ),
            Err(IncidenceError::InvalidWeight { tuple_index: 1, .. })
        ));
        assert!(matches!(
            ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&[invalid])),
            Err(IncidenceError::InvalidWeight { tuple_index: 0, .. })
        ));
    }
    assert!(matches!(
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Observations(&[f64::MAX; 2])),
        Err(IncidenceError::InvalidCollapsedWeight { .. })
    ));
    // Distinct finite tuple totals can still overflow a shared factor's degree.
    let shared =
        PreparedThreeWayTopology::try_from_collapsed([1, 2, 2], &[[0, 0, 0], [0, 1, 1]]).unwrap();
    assert!(matches!(
        ThreeWayWeightFrame::try_new(&shared, WeightFrameInput::Tuples(&[f64::MAX; 2])),
    ...
    let frame = ThreeWayWeightFrame::try_new(
        &disconnected,
        WeightFrameInput::Tuples(&[f64::from_bits(1), f64::MAX]),
    )
    .unwrap();
...
fn checked_budget_counts_old_new_overlap_and_input_storage() {
    ...
    assert_eq!(
        report.topology_payload_bytes,
        topology.retained_payload_bytes().unwrap()
    );
    ...
    assert_eq!(
        ...
        report.topology_payload_bytes + 8 + report.new_arrays_payload_bytes + retained
    );
    let build = |limit| {
        ThreeWayWeightFrame::try_new_with_budget(
            ...
            WeightFramePayloadBudget {
                ...
                additional_live_payload_bytes: retained,
            },
        )
    };
    assert!(matches!(
        build(report.total_payload_bytes - 1),
        Err(IncidenceError::WeightFrameBudgetExceeded { .. })
    ));
    let new = build(report.total_payload_bytes).unwrap();
    bits(old.weights(), &[1.0]);
    bits(new.weights(), &[3.0]);
    let alias = ThreeWayWeightFrame::setup_payload_report(
        &topology,
        WeightFrameInput::Tuples(old.weights()),
        retained,
    )
    .unwrap();
    ...
    assert!(
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&[1.0, 2.0])).is_err()
    );
}
...
fn topology_borrows_and_numerical_frames_can_be_shared_across_threads() {
    ...
    std::thread::scope(|scope| {
        ...
        let b = scope.spawn(|| {
            let mut output = [0.0];
            first
                .copy_weights_into(&topology, first.binding(), &mut output)
                .unwrap();
            output[0]
        });
...
fn published_frame_owns_its_values_independently_of_the_input_slice() {
    ...
    let frame =
        ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Observations(&values)).unwrap();
    values.fill(f64::NAN);

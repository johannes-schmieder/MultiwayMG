...
fn full(counts: [u32; 3]) -> Vec<[u32; 3]> {
    (0..counts[0])
        .flat_map(|i| (0..counts[1]).flat_map(move |j| (0..counts[2]).map(move |k| [i, j, k])))
        .collect()
}
...
fn check_coarse(map: &PreparedCoarseTupleMap<'_>, parent: &ThreeWayWeightFrame<'_>) {
    ...
    assert_eq!(
        replay.frame().topology().component_labels(),
        fresh.components().labels()
    );
    bits(replay.frame().weights(), fresh.weights());
    bits(
        replay.frame().square_root_weights(),
        fresh.square_root_weights(),
    );
    ...
    for (&tuple, &weight) in coarse.tuples().iter().zip(&expected) {
        for f in 0..3 {
            add(&mut degrees[coarse.global_index(f, tuple[f])], weight);
        }
    }
...
fn check_pair(map: &PreparedPairEdgeMap<'_>, parent: &ThreeWayWeightFrame<'_>) {
    ...
    replay
        .copy_conductances_into(map, parent, &mut copied)
        .unwrap();
...
fn raw_duplicates_changing_frames_chaining_and_old_generation_retention() {
    ...
    let pairs: Vec<_> = FactorPair::ALL
        .into_iter()
        .map(|p| PreparedPairEdgeMap::try_new(&t, p).unwrap())
        .collect();
    ...
    for step in 0..32 {
        ...
        assert!(matches!(
            old_replay.validate_for(&map, &f),
            Err(IncidenceError::WeightFrameBindingMismatch)
        ));
        assert!(matches!(
            old_pair.validate_for(&pairs[0], &f),
            Err(IncidenceError::WeightFrameBindingMismatch)
        ));
    ...
    std::thread::scope(|s| {
        let c = s.spawn(|| {
            CoarseWeightReplay::try_new(&map, &old)
                .unwrap()
                .frame()
                .weights()
                .to_vec()
        ...
        let p = s.spawn(|| {
            PairConductanceReplay::try_new(&pairs[0], &old)
                .unwrap()
                .conductances()
                .to_vec()
        });
...
fn wrong_equal_maps_frames_and_dimensions_reject_before_any_output_writes() {
    ...
    assert!(matches!(
        CoarseWeightReplay::try_new(&c1, &wrong),
        Err(IncidenceError::TopologyBindingMismatch)
    ));
    assert!(matches!(
        PairConductanceReplay::try_new(&p1, &wrong),
        Err(IncidenceError::TopologyBindingMismatch)
    ));
    ...
    for parent in [&equal, &changed, &wrong] {
        assert!(
            coarse
                .copy_weights_into(&c1, parent, &mut out[..1])
                .is_err()
        );
    ...
    assert!(matches!(
        coarse.copy_weights_into(&c2, &f, &mut out[..1]),
        Err(IncidenceError::WeightReplayMapMismatch)
    ));
    for map in [&p2, &p3] {
        assert!(matches!(
            pair.copy_conductances_into(map, &f, &mut out),
            Err(IncidenceError::WeightReplayMapMismatch)
        ));
...
fn payload_budget_charges_direct_owners_and_declared_old_new_overlap() {
    ...
    for report in [cr, pr] {
        assert_eq!(
            report.source_topology_payload_bytes,
            t.retained_payload_bytes().unwrap()
        );
        assert_eq!(
            report.source_frame_payload_bytes,
            f.retained_payload_bytes().unwrap()
        );
        assert_eq!(report.additional_live_payload_bytes, extra);
        assert_eq!(
            report.total_payload_bytes,
            report.source_topology_payload_bytes
                + report.symbolic_map_payload_bytes
                + report.aggregation_payload_bytes
                + report.source_frame_payload_bytes
    ...
    assert_eq!(
        cr.aggregation_payload_bytes,
        a.retained_payload_bytes().unwrap()
    );
    assert_eq!(pr.aggregation_payload_bytes, 0);
    let budget = |n| WeightReplayPayloadBudget {
        maximum_payload_bytes: n,
        additional_live_payload_bytes: extra,
    };
    assert!(
        CoarseWeightReplay::try_new_with_budget(&c, &f, budget(cr.total_payload_bytes - 1))
            .is_err()
    );
    CoarseWeightReplay::try_new_with_budget(&c, &f, budget(cr.total_payload_bytes)).unwrap();
    assert!(
        PairConductanceReplay::try_new_with_budget(&p, &f, budget(pr.total_payload_bytes - 1))
            .is_err()
    );

...
fn compensated_pair_protocol_differs_explicitly_from_legacy_ordinary_addition() {
    ...
    let frame = ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&weights)).unwrap();
...
fn pair_degrees_are_rebuilt_from_represented_conductances_not_fine_degrees() {
    ...
    let frame = ThreeWayWeightFrame::try_new(&topology, WeightFrameInput::Tuples(&[
        9_007_199_254_740_990.0, 0.5, 0.53125,
    ])).unwrap();
    ...
    assert_ne!(replay.diagonal()[0].to_bits(), frame.diagonal()[0].to_bits());
}
...
fn valid_fine_frames_can_reject_coarse_totals_coarse_degrees_or_pair_degrees() {
    let topology = PreparedThreeWayTopology::try_from_collapsed(
        [2; 3], &[[0, 0, 0], [0, 1, 1], [1, 1, 1]],
    ).unwrap();
    let frame = ThreeWayWeightFrame::try_new(&topology,
        WeightFrameInput::Tuples(&[f64::MAX * 0.75, 1.0, f64::MAX * 0.75])).unwrap();
    ...
    assert!(matches!(CoarseWeightReplay::try_new(&total_map, &frame),
        Err(IncidenceError::InvalidWeightFrameDerivedValue { context: "coarse tuple total", index: 0, .. })));
    ...
    assert!(matches!(CoarseWeightReplay::try_new(&degree_map, &frame),
        Err(IncidenceError::InvalidWeightedDegree { factor: 0, level: 0, .. })));
    ...
    let pair_topology = PreparedThreeWayTopology::try_from_collapsed(
        [1, 2, 2], &[[0, 0, 0], [0, 0, 1], [0, 1, 0]],
    ).unwrap();
    let ulp = 2.0_f64.powi(971);
    let pair_frame = ThreeWayWeightFrame::try_new(&pair_topology,
        WeightFrameInput::Tuples(&[f64::from_bits(f64::MAX.to_bits() - 1), ulp * 0.5, ulp * 0.53125])).unwrap();
    assert_eq!(pair_frame.diagonal()[0], f64::MAX);
    let pair_map = PreparedPairEdgeMap::try_new(&pair_topology, FactorPair::OneThree).unwrap();
    assert!(matches!(PairConductanceReplay::try_new(&pair_map, &pair_frame),
        Err(IncidenceError::InvalidWeightFrameDerivedValue { context: "pair weighted degree", index: 0, .. })));
...
fn single_extreme_weights_and_disconnected_pair_support_are_preserved() {
    ...
    let t = PreparedThreeWayTopology::try_from_collapsed([2, 2, 1], &[[0, 0, 0], [1, 1, 0]]).unwrap();
...
fn replayed_pair_and_coarse_matrices_match_original_operator_algebra_on_dyadic_weights() {
    let tuples: Vec<_> = (0..3).flat_map(|i| (0..4).flat_map(move |j| (0..2).map(move |k| [i, j, k]))).collect();
    ...
    for pair in FactorPair::ALL {
        ...
        let indices: Vec<_> = t.topology().factor_range(l).chain(t.topology().factor_range(r)).collect();
        let mut matrix = vec![vec![0.0; n]; n];
        for (i, &d) in replay.diagonal().iter().enumerate() { matrix[i][i] = d; }
        ...
        for i in 0..n { for j in 0..n {
            let sign = if (i < map.level_counts()[0]) == (j < map.level_counts()[0]) { 1.0 } else { -1.0 };
    ...
    let coarse = ThreeWayProblem::from_observations(ct.level_counts(), ct.tuples(), replay.frame().weights()).unwrap().dense_gramian();
    let parents: Vec<_> = (0..3).flat_map(|factor|
        a.parents(factor).iter().map(move |&p| ct.global_index(factor, p))).collect();
    let mut galerkin = vec![vec![0.0; ct.total_levels()]; ct.total_levels()];
    for i in 0..fine.dimension() { for j in 0..fine.dimension() { galerkin[parents[i]][parents[j]] += gramian[i][j]; }}

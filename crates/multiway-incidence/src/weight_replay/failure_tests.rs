...
use super::*;
use crate::{
    FactorAggregation, FactorPair, PreparedCoarseTupleMap, PreparedPairEdgeMap, WeightFrameInput,
};
    ...
    for unwind in [false, true] {
        for fail_at in 0..5 {
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                CoarseWeightReplay::build_with(
                    &c,
                    &f,
                    WeightReplayPayloadBudget::UNLIMITED,
                    &mut |context| {
                        let index = reached;
                        reached += 1;
                        if index == fail_at {
        ...
        for pair in FactorPair::ALL {
            ...
            for fail_at in 0..3 {
                let mut reached = 0;
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    PairConductanceReplay::build_with(
                        &p,
                        &f,
                        WeightReplayPayloadBudget::UNLIMITED,
                        &mut |context| {
...
fn wrong_topology_insufficient_budget_and_integer_overflow_precede_reservations() {
    ...
    assert!(
        CoarseWeightReplay::build_with(
            ...
            &wrong,
            WeightReplayPayloadBudget::UNLIMITED,
            &mut |_| panic!("owner must reject first")
        )
        .is_err()
    );
    assert!(
    ...
    for budget in [
        ...
        assert!(
            CoarseWeightReplay::build_with(&c, &f, budget, &mut |_| panic!("budget first"))
                .is_err()
        );
        assert!(
            PairConductanceReplay::build_with(&p, &f, budget, &mut |_| panic!("budget first"))
                .is_err()
        );
    }
}

...
matrix. [Opt-in kernel profiling](docs/ISSUE5_KERNEL_PROFILING.md) now supplies
...
[fused transfer increment](docs/ISSUE5_FUSED_TRANSFER.md) removes one temporary store/read
...
timing gain is claimed. The [single-arena workspace](docs/ISSUE5_TRAVERSAL_ARENA.md)
...
accounting retains all prior evidence. The [certificate liveness increment](docs/ISSUE5_CERTIFICATE_LIVENESS.md)
...
[Flat component permutations](docs/ISSUE5_COMPONENT_LAYOUT.md)
...
[Explicit adjacent-pair candidates](docs/ISSUE5_ADJACENT_CANDIDATES.md)
...
work. [Explicit grouped automatic execution](docs/ISSUE5_AUTOMATIC_GROUPED_LAYOUTS.md)
...
decisions remain open in M6. [Separate automatic cost diagnostics](docs/ISSUE5_AUTOMATIC_DIAGNOSTICS.md)
...
[dense reconstruction increment](docs/ISSUE5_DENSE_RECONSTRUCTION.md) streams
...
[canonical pair-order increment](docs/ISSUE5_CANONICAL_PAIR_ORDER.md) replaces one redundant sort with a reset
...
real factors and first/repeated allocations, plus 160 downstream RHS and all
15,120 unchanged Linux smoke signatures. Factor-memory and numerical-range
